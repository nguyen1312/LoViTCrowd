...
#         print("Tran Hoang Nguyen: ", attn.size())
...
#         print("Tran Hoang Nguyen_v1: ", attn.size())

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
#         print("Attention_weight: ", x.size())
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, weight
...
#         print("Meow meow")
        ...
        weight_attn = self.attn(self.norm1(x))[1]
#         print(x.size())
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
#         print(x.size())
#         print("Gau gau")
        return x, weight_attn
...
#             print("w: ", attn_w.size())
            attn_ws_12_layer.append(attn_w)
        x = x + x1
        x = self.norm(x)
        x = x[:, 1:]
        return x, attn_ws_12_layer
    def forward(self, x):
        x, attn_ws = self.forward_features(x)
...
def base_patch16_384_extractor(pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('/vinbrain/nguyentran/03.Train/crowd_estimation/code/pretrained_vit/jx_vit_base_p32_384-830016f5.pth')
